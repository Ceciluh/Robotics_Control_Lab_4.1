...
def get_qpos_adr(m, joint_names):
    ids = [mujoco.mj_name2id(m, mujoco.mjtObj.mjOBJ_JOINT, jn) for jn in joint_names]
    return [m.jnt_qposadr[jid] for jid in ids]
...
# ---------------------------
def move_to_pose(m, d, viewer, target_pose, duration=2.0, realtime=True, log=None, qpos_adr=None, joint_names=None, plotter=None):
    ...
    while viewer.is_running():
        ...
        if log is not None and qpos_adr is not None and joint_names is not None:
            q_act = [float(d.qpos[idx]) for idx in qpos_adr]
            log.append([float(d.time)] + q_act)
...
def hold_position(m, d, viewer, duration=2.0, realtime=True, log=None, qpos_adr=None, joint_names=None, plotter=None):
